...
<section class="bg-murky-900 mb-8">
  ...
  <?php endif; ?>
  <p class="text-xl font-semibold text-left rtl:text-right text-gray-900 dark:text-white">Data Pembelian</p>
...
$(document).ready(function () {
    var table = $('#table').DataTable({
        ...
        "pagingType": 'simple',
        "ajax": {
            "url": "<?php echo base_url('admin/pembelian/getPembelian'); ?>",
            "type": "POST"
        },
        "columns": [
            ...
            {"data": null, "render": function (data, type, row) {
                return '<div class="flex"><a href="<?= base_url('admin/pembelian/edit/') ?>' + row.id + '" class="focus:outline-none text-white bg-yellow-400 hover:bg-yellow-500 font-medium rounded-lg text-sm px-3 py-1 me-2">Edit</a>' +
                    '<a href="<?= base_url('admin/pembelian/hapus/') ?>' + row.id + '" type="button" class="focus:outline-none text-white bg-red-700 hover:bg-red-800 font-medium rounded-lg text-sm px-3 py-1 me-2" data-id="' + row.id + '">Hapus</a></div>';
            }}
        ]
