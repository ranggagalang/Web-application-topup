...
<section class="bg-murky-900 mb-8">
  ...
  <?php endif; ?>
  <p class="text-xl font-semibold text-left rtl:text-right text-gray-900 dark:text-white">Data Kategori</p>
          ...
          <tbody>
              <?php foreach ($kategori as $index => $k): ?>
                  <tr class="whitespace-nowrap">
                      ...
                      <td class="px-6 py-4">
        								<div class="flex">
        									<button type="button" class="focus:outline-none text-white bg-red-700 hover:bg-red-800 font-medium rounded-lg text-sm px-3 py-1 me-2" data-modal-target="hapus-modal<?= $k['id'] ?>" data-modal-toggle="hapus-modal<?= $k['id'] ?>">Hapus</button>
        								</div>		
        							</td>
                  </tr>
              <?php endforeach; ?>
              ...
              <form action="<?= base_url('admin/kategori/tambah') ?>" method="post" enctype="multipart/form-data">
                <?= csrf_field() ?>
                  <div class="mb-3">
                    ...
                    <input type="text" name="nama" class="custom-input block w-full" placeholder="Masukkan nama kategori" required>
                  </div>
...
<?php foreach ($kategori as $key => $k): ?>
  ...
  <div id="hapus-modal<?= $k['id'] ?>" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <div class="relative p-4 w-full max-w-md max-h-full">
        <div class="relative bg-murky-800 rounded-lg shadow">
            <button type="button" class="absolute top-3 end-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="hapus-modal<?= $k['id'] ?>">
                <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                    <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6"/>
                </svg>
                <span class="sr-only">Close modal</span>
            ...
            <div class="p-4 md:p-5 text-center">
                ...
                <h3 class="mb-5 text-lg font-normal text-white">Apakah Anda yakin ingin menghapus data kategori ini?</h3>
                <button onclick="window.location.href='<?= base_url('admin/kategori/hapus/' . $k['id']) ?>'" type="button" class="text-white bg-red-600 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center">
                    Hapus
                </button>
                <button data-modal-hide="hapus-modal<?= $k['id'] ?>" type="button" class="py-2.5 px-5 ms-3 text-sm font-medium text-white focus:outline-none bg-murky-700 rounded-lg">Batal</button>
            </div>
        </div>
    </div>
</div>
